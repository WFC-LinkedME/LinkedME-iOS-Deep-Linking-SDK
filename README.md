...
-(void)addPara{
    ...
    [self.linkedMEUniversalObject getShortUrlWithLinkProperties:linkProperties andCallback:^(NSString *url, NSError *err) {
        //  这里的url就是得到的短链 
        if (err == nil) {
...
| **参数** | **含义**
| --- | ---
| ~channel | 用于区分连接的渠道如：微信，微博，短信，邮件，其它等等
|~feature|业务名称
| ~tags | 创建连接时添加的标签信息
| ~campaign | 链接关联的活动
| +is_first_session | [Boolean]是否为首次会话/(打开还是安装)
| +clicked_linkedme_link | 用户是否点击LinkedME连接
| +timestamp | 当前时间戳
|~My Content Description|内容说明
|~alias|别名
